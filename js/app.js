...
function fixMath(txt) {
  ...
  var s = txt;
  // Greek
  var g = {alpha:'α',beta:'β',gamma:'γ',delta:'δ',epsilon:'ε',theta:'θ',
    lambda:'λ',mu:'μ',pi:'π',rho:'ρ',sigma:'σ',tau:'τ',varphi:'φ',chi:'χ',omega:'ω',
    Sigma:'Σ',Pi:'Π',Omega:'Ω',Phi:'Φ',Theta:'Θ',Lambda:'Λ',Gamma:'Γ',Delta:'Δ'};
  for (var k in g) s = s.split('\\\\'+k).join(g[k]);
  // Math symbols
  ...
  s = s.split('\\\\in').join('∈');
  s = s.split('\\\\subset').join('⊂');
  s = s.split('\\\\cup').join('∪');
  s = s.split('\\\\cap').join('∩');
  s = s.split('\\\\sum').join('Σ');
  s = s.split('\\\\int').join('∫');
  s = s.split('\\\\prod').join('Π');
  s = s.split('\\\\sqrt').join('√');
  // frac{a}{b} -> (a)/(b)
